...
from blog.users.forms import RegistrationForm, LoginForm, UpdateForm, RequestResetForm, ResetPasswordForm
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('An email has been sent to your registered Email with intructions to reset your password', 'info')
        return redirect(url_for('users.login'))
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Your Password has been changed! and you are now able to Login', 'success')
        return redirect(url_for('users.login')) 
